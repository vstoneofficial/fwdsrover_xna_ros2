...
def generate_launch_description():
    ...
    spawn_wall = IncludeLaunchDescription(
        ...
        condition=IfCondition(PythonExpression(['"', wall, '" != "none" and "', wall, '" != ""'])),
        launch_arguments={
            'wall': wall,
        }.items()
    )
    ...
    # SLAM有効でNav2を起動（map_server/amclは使用しない）
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('rvizconfig', default_value=PathJoinSubstitution([nav_pkg, 'rviz', 'nav2.rviz']),
                              description='RViz config file (default: nav2.rviz)'),
