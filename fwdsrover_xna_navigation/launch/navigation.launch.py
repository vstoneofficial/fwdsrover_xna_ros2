...
def generate_launch_description():
    bringup_dir = get_package_share_directory('fwdsrover_xna_navigation')
    ...
    selected_params_file = PythonExpression([
        '"', params_file, '" if "', params_file, '" != "" else '
        '("', bringup_dir, '/config/x40a_nav2_params.yaml" if "', rover,
        '" == "x40a" else "', bringup_dir, '/config/x120a_nav2_params.yaml")'
    ])
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('rvizconfig', default_value=PathJoinSubstitution([FindPackageShare('fwdsrover_xna_navigation'), 'rviz', 'nav2.rviz']), description='Absolute path to rviz config file'),
