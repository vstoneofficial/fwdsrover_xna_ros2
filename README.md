...
2. Nav2でナビゲーションする。
   ...
   注意点:
- ナビゲーションを行う場合、移動命令のrover_twistトピックが競合しないようにするため、
　mouse_teleop.launch.py、rover_gamepad.launch.pyなどのrover_twistをpublishするノードは起動しないでください。
...
wall:=Wall.stlの部分は、fwdsrover_xna_gazebo/meshes内にあるSTLファイルに書き換えることでシミュレータ内の壁としてインポートすることができます。
