...
- [使用方法](#%E4%BD%BF%E7%94%A8%E6%96%B9%E6%B3%95)
    ...
    - [4WDSローバー（実機）との通信](#4wds%E3%83%AD%E3%83%BC%E3%83%90%E3%83%BC%E5%AE%9F%E6%A9%9F%E3%81%A8%E3%81%AE%E9%80%9A%E4%BF%A1)
    ...
    - [gazeboシミュレーションの準備](#gazeboシミュレーションの準備)
    - [gazeboでシミュレーションする。](#gazeboでシミュレーションする)
...
## ファイルの構成
   ```
    ...
    │　├ fwdsrover_xna_bringup
    │　├ fwdsrover_xna_gazebo
    │　└ fwdsrover_xna_navigation
...
- `fwdsrover_xna_navigation`: 4WDSローバーのSLAMやnavigationに関連するノードやlaunchファイルを含んでいるパッケージです。
...
### 4WDSローバー（実機）との通信
ROS 2とMicro-ROSを統合するためのエージェントノードを起動します。
#### ● 有線シリアル接続の場合:
   ```
...
- LRFに関するパラメータは[TG30.yaml](./fwdsrover_xna_bringup/params/TG30.yaml)にあります。  
> **Warning**  
...
#### Nav2でナビゲーションする。  
   ...
   - launchファイル内で指定する方法  
      ...
      [`navigation.launch.py`](./fwdsrover_xna_navigation/launch/navigation.launch.py) を開き、`map` の既定値を、先ほど保存・移動させた地図ファイルのファイル名に合わせて設定します。
      ```
...
2. 新しいターミナルで以下のコマンドを使用してナビゲーションを開始します。  
   map:=... の部分に、使用したい地図ファイルのフルパス（`.yaml`まで）を指定することで、navigation.launch.py内を書き換えずに起動することも可能です。
   ```
   ros2 launch fwdsrover_xna_navigation navigation.launch.py rover:=x40a map:=~/ros2_ws/src/fwdsrover_xna_ros2/fwdsrover_xna_navigation/maps/YOUR_MAP_NAME.yaml
   ```
...
引数で地図ファイルのパスを指定する方法と、launchファイル内で地図の名前を指定する方法があります。
   ...
   [`gazebo_nav.launch.py`](./fwdsrover_xna_gazebo/launch/gazebo_nav.launch.py) を開き、`map` の既定値を、先ほど保存した地図ファイルの名前に合わせて設定します。  
   ...
   map:=... の部分に、使用したい地図ファイルのフルパス（`.yaml`まで）を指定することで、gazebo_nav.launch.py内を書き換えずに起動することも可能です。
      ```
      ros2 launch fwdsrover_xna_gazebo gazebo_nav.launch.py rover:=x40a wall:=Wall.stl map:=~/ros2_ws/src/fwdsrover_xna_ros2/fwdsrover_xna_gazebo/maps/YOUR_MAP_NAME.yaml
      ```
