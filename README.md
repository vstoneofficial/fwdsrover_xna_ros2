...
- `fwdsrover_xna_ros2_description`: 4WDSローバーの物理モデルやURDFモデルを含んでいるパッケージです。
...
### odometryをpublish
 pub_odomノードとrviz上可視化
   ...
   ```
   $ ros2 launch fwdsrover_xna_bringup robot.launch.py mode:=odo_driven
   ```
...
3. SLAM ToolboxでSLAMする。
   - ロボット実機と通信できたら、以下のコマンドでodom publisher、ロボットのurdf表示、LiDAR関連のlaunchファイルを起動します。
      ...
      ```
      ros2 launch fwdsrover_xna_bringup nav_robot.launch.py
      ```

   - 新しいターミナルで以下のコマンドを使用してSLAMを開始します。

      ```
      ros2 launch fwdsrover_xna_bringup slam.launch.py
      ```
...
2. Nav2でナビゲーションする。
   - [`navigation.launch.py`](./fwdsrover_xna_navigation/launch/navigation.launch.py#L35) の行35のマップ名を使用したいマップ名に変更する。
   ...
   - 新しいターミナルで以下のコマンドを使用してナビゲーションを開始します。
      ...
      ```
      ros2 launch fwdsrover_xna_navigation navigation.launch.py
      ```

   - 4WDSローバー用のNavigation2のパラメータは`fwdsrover_xna_navigation`パッケージの[`config/nav2_params.yaml`](./fwdsrover_xna_navigation/config/nav2_params.yaml) フォルダにあります。

### gazeboを使用したシミュレーション
1. gazeboパッケージをインストールする。

   ```
   sudo apt install ros-$ROS_DISTRO-gazebo-msgs ros-$ROS_DISTRO-gazebo-ros-pkgs
...
1. コントローラノードを起動します。
   ...
   ```
   ros2 launch fwdsrover_xna_bringup rover_sim.launch.py
   ```

2. 空のワールドを起動します。

   ```
   ros2 launch fwdsrover_xna_gazebo gazebo_bringup.launch.py
   ```

3. 壁をシミュレータ内にインポートします

   ```
   ros2 launch fwdsrover_xna_gazebo spawn_wall.launch.py
   ```

- ローバーをマウスで遠隔操作するには、新しいターミナル ウィンドウで以下のコマンドを使用して遠隔操作ノードを起動します。
...
2. gazebo serverとSLAMノードを起動します。

   ```
   ros2 launch fwdsrover_xna_gazebo gazebo_slam.launch.py 
   ```
...
#### gazeboでナビゲーションする。
- [`gazebo_nav.launch.py`](./fwdsrover_xna_gazebo/launch/gazebo_nav.launch.py#L35) の行35のマップ名を使用したいマップ名に変更する。
   　 次の行を探し、“ファイル名.yaml”を、先ほど保存，移動させた地図ファイルのファイル名に合わせて設定します。
...
2. gazebo serverとNav2ノードを起動します。

   ```
   ros2 launch fwdsrover_xna_gazebo gazebo_nav.launch.py 
   ```
